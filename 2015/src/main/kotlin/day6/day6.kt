...
class LightGrid {
  ...
  fun configureLightsQ1(instruct: Instruction) {
    for (x in instruct.start.first..instruct.end.first) {
      for (y in instruct.start.second..instruct.end.second) {
        lightGrid[Pair(x, y)] = when (instruct.action) {
          LightAction.TURN_OFF -> 0
          LightAction.TURN_ON -> 1
          LightAction.TOGGLE -> if (lightGrid[Pair(x, y)]!! == 0) 1 else 0
        }
  ...
  fun configureLightsQ2(instruct: Instruction) {
    for (x in instruct.start.first..instruct.end.first) {
      for (y in instruct.start.second..instruct.end.second) {
        lightGrid[Pair(x, y)] = when (instruct.action) {
